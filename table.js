...
d3.csv('data1.csv').then((data) => {
    ...
    const locationGrouped = d3.group(data, (d) => d.setting, (d) => d.continent, (d) => d.st_design);
...
legend.append("text").attr("x", 12).attr("y", 80).text("A single study may report more than one region or sector, so study count may exceed the number of studies characterized.").style("font-size", "15px").attr("alignment-baseline","middle")